...
# We calculate the average spend for each user up to that point in time
df['avg_spend'] = df.groupby('user_id')['amount'].transform(lambda x: x.expanding().mean())
...
print(df[['user_id', 'amount', 'avg_spend', 'is_fraud']])
...
```text
--- Training Output ---
   user_id  amount  avg_spend  is_fraud
0      101   100.0      100.0         0
1      101   200.0      150.0         0
2      102    50.0       50.0         0
3      101   150.0      150.0         1
